...
describe('search center page', () => {
  ...
  it('sets the state to the value of input field', () => {
    wrapper.find('.input-search').simulate('change', { target: { name: 'searchQueryValue', value: 'New' } });
    expect(wrapper.state().searchQueryValue).toEqual('New');
  });

  it('shows an error prompt if you click on search and input field is empty', () => {
    wrapper.find('.input-search').simulate('change', { target: { name: 'searchQueryValue', value: '' } });
    wrapper.find('.btn-search-l').simulate('click');
    expect(props.actionRejectedPrompter).toHaveBeenCalledWith('search field cannot be empty');
  });

  it('shows an error prompt if you click on search and input field is empty', () => {
    wrapper.find('.input-search').simulate('change', { target: { name: 'searchQueryValue', value: 'Ne' } });
    wrapper.find('.btn-search-l').simulate('click');
    expect(props.actionRejectedPrompter).toHaveBeenCalledWith('Please select the type of search');
  });

  it('searches by name or rentalCost if corresponding radio button is clicked', () => {
    wrapper.find('.radio-name').simulate('click');
    wrapper.find('.btn-search-l').simulate('click');
    expect(props.getAllCenters).toHaveBeenCalled();
  });

  it('redirects to the modifycenter page if modification button is clicked', () => {
